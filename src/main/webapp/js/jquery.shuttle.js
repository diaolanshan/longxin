        ...
        init: function(opts) {
            ...
            return this.each(function() {
               ...
               /* 複製來源select名稱並加上-dst到目標select */
                ...
                .append('<input type="button" id="shuttle-control-dst-src" class="ui-corner-all ui-state-default" value="&lsaquo;" />')
                .append('<input type="button" id="shuttle-control-all-src-dst" class="ui-corner-all ui-state-default" value="&raquo;" />')
                .append('<input type="button" id="shuttle-control-all-dst-src" class="ui-corner-all ui-state-default" value="&laquo;" />');

                /* 帶入原始長寬到來源和目標select */
                $input.css('width', $src.width());
                $dst.css('width', "200px");
                $dst.css('height', "150px");
                ...
                $controls.find('#shuttle-control-src-dst').on('click', function(event) {
                    $.fn.shuttle.transfer($src, $dst, false);
                });

                /* 將選擇的目標資料列移至來源資料列 */
                $controls.find('#shuttle-control-dst-src').on('click', function(event) {
                    $.fn.shuttle.transfer($dst, $src, false);
                });

                /* 將全部來源資料列移至目標資料列 */
                $controls.find('#shuttle-control-all-src-dst').on('click', function(event) {
                    $.fn.shuttle.transfer($src, $dst, true);
                });

                /* 將全部目標資料列移至來源資料列 */
                $controls.find('#shuttle-control-all-dst-src').on('click', function(event) {
                    $.fn.shuttle.transfer($dst, $src, true);
                });
                
                /* 將來源select的所有選項傳入data-options中,用來篩選欄位時使用 */
                var $options = [];
